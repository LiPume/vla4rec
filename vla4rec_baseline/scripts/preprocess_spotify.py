...
with open(output_file, 'w') as f:
    # 遍历 1000 个 slice 文件for filename in tqdm(sorted(os.listdir(data_dir))):
        if filename.endswith('.json'):
            with open(os.path.join(data_dir, filename), 'r') as j:
                data = json.load(j)
                for playlist in data['playlists']:
                    # 过滤逻辑：只保留长度在 10-100 之间的优质“专家轨迹”if 10 <= len(playlist['tracks']) <= 100:
                        track_ids = []
                        for track in playlist['tracks']:
                            uri = track['track_uri']
                            # 建立 URI 到整数 ID 的映射 (Action Tokenization)if uri not in uri_to_id:
...
# 保存映射表，这相当于我们的 Action Vocabularywith open(mapping_file, 'w') as m:
